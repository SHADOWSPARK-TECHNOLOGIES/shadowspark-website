...
describe("GET /api/cron/listings/expiry", () => {
  ...
  it("fails closed when CRON_SECRET is blank", async () => {
    process.env.CRON_SECRET = "   ";

    const response = await invokeCron("Bearer    ");

    expect(response?.status).toBe(401);
    expect(mocks.listingFindMany).not.toHaveBeenCalled();
  });

  it("rejects a missing authorization header without database work", async () => {
    const response = await invokeCron();
  ...
  it("is idempotent on repeated authorized expiry invocation", async () => {
    mocks.listingUpdateMany
      .mockResolvedValueOnce({ count: 2 })
      .mockResolvedValueOnce({ count: 0 });

    const first = await invokeCron("Bearer test-cron-secret");
    const second = await invokeCron("Bearer test-cron-secret");

    expect(await first?.json()).toMatchObject({ deactivated: 2 });
    expect(await second?.json()).toMatchObject({ deactivated: 0 });
    expect(mocks.listingUpdateMany).toHaveBeenCalledTimes(2);
    expect(mocks.listingUpdateMany).toHaveBeenNthCalledWith(2, {
      where: { active: true, expiresAt: { lt: now } },
      data: { active: false },
    });
  });
